...
outpath: path where to put generated data.h5
"""
        ...
        for channel in event.data.dtype.names:
            for i, adc in enumerate(event.data[pixel][channel][min_sample_id:max_sample_id]):
                if adc >= min_adc:
                    ...
                    spikes.append([adc, pixel, channel, event.header.event_counter, cell_id, sample_id, event.time_since_last_readout[pixel][channel][sample_id]])
    return spikes
...
def remove_nones_and_flatten(data):
    """Remove empty entries from list and expand lists of lists to single list entries"""
...
if __name__ == '__main__':
    ...
    data = remove_nones_and_flatten(data)
